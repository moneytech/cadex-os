...
int proc_run(const char *cmd, int argc, const char **argv) {
    return syscall(SYSCALL_PROCESS_RUN, (uint32_t)cmd, argc, (uint32_t)argv, 0,
                   0);
}

int syscall_process_wrun(const char *cmd, int argc, const char **argv, int *fds,
                         int fd_len) {
    return syscall(SYSCALL_PROCESS_WRUN, (uint32_t)cmd, argc, (uint32_t)argv,
                   (uint32_t)fds, fd_len);
}

int fork() { return syscall(SYSCALL_PROCESS_FORK, 0, 0, 0, 0, 0); }
...
int syscall_open_file_relative(int fd, const char *path, int mode,
                               kernel_flags_t flags) {
    return syscall(SYSCALL_OPEN_FILE_RELATIVE, fd, (uint32_t)path, mode, flags,
                   0);
}
...
int syscall_open_pipe() { return syscall(SYSCALL_OPEN_PIPE, 0, 0, 0, 0, 0); }
...
int read_object_nonblock(int fd, void *data, int length) {
    return syscall(SYSCALL_OBJECT_READ_NONBLOCK, fd, (uint32_t)data, length, 0,
                   0);
}

int syscall_object_list(int fd, char *buffer, int n) {
    return syscall(SYSCALL_OBJECT_LIST, fd, (uint32_t)buffer, (uint32_t)n, 0,
                   0);
}
...
int syscall_object_get_tag(int fd, char *buffer, int buffer_size) {
    return syscall(SYSCALL_OBJECT_GET_TAG, fd, (uint32_t)buffer, buffer_size, 0,
                   0);
}
...
int syscall_object_max() { return syscall(SYSCALL_OBJECT_MAX, 0, 0, 0, 0, 0); }
...
int syscall_device_driver_stats(char *name, void *stats) {
    return syscall(SYSCALL_DEVICE_DRIVER_STATS, (uint32_t)name, (uint32_t)stats,
                   0, 0, 0);
}
