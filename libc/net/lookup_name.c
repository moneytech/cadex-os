...
static int name_from_null(struct address buf[static 2], const char *name,
...
static int name_from_numeric(struct address buf[static 1], const char *name,
			     int family) {
  return __lookup_ipliteral(buf, name, family);
}

static int name_from_hosts(struct address buf[static MAXADDRS],
			   char canon[static 256], const char *name,
			   int family) {
  char line[512];
  ...
  while (read_object(line, sizeof line, f) && cnt < MAXADDRS) {
    char *p, *z;
...
static int dns_parse_callback(void *c, int rr, const void *data, int len,
			      const void *packet) {
  char tmp[256];
...
static int name_from_dns(struct address buf[static MAXADDRS],
  ...
  for (i = 0; i < 2; i++) {
    if (family != afrr[i].af) {
      qlens[nq] = __res_mkquery(0, name, 1, afrr[i].rr, 0, 0, 0, qbuf[nq],
				sizeof *qbuf);
      if (qlens[nq] == -1)
	return EAI_NONAME;
      nq++;
...
int __lookup_name(struct address buf[static MAXADDRS], char canon[static 256],
  ...
  *canon = 0;
  ...
  for (i = 0; i < cnt; i++) {
    ...
    int key = 0;
    struct sockaddr_in6 sa6 = {0}, da6 = {.sin6_family = AF_INET6,
					  .sin6_scope_id = buf[i].scopeid,
					  .sin6_port = 65535};
    struct sockaddr_in sa4 = {0},
		       da4 = {.sin_family = AF_INET, .sin_port = 65535};
    void *sa, *da;
	...
	if (!getsockname(fd, sa, &salen)) {
	  if (family == AF_INET)
	    memcpy(sa6.sin6_addr.s6_addr + 12, &sa4.sin_addr, 4);
