...
void (*symbol_find(const char *name))(void) {
    ...
    while ((uintptr_t)k < (uintptr_t)&kernel_symbols_end) {
        if (strcmp(k->name, name)) {
            k = (kernel_symbol_t *)((uintptr_t)k + sizeof *k + strlen(k->name) +
                                    1);
            continue;
        }
...
void debug_print(char *print) { printf(print); }
int module_quickcheck(void *blob) {

    Elf32_Header *target = (Elf32_Header *)blob;
    char *head = (char *)blob;

    if (target->e_ident[0] != ELFMAG0 || target->e_ident[1] != ELFMAG1 ||
        target->e_ident[2] != ELFMAG2 || target->e_ident[3] != ELFMAG3) {
...
void *module_load_direct(void *blob, size_t length) {
    ...
    /* TODO: Actually load the ELF somewhere! This is moronic, you're not
     * initializing a BSS! */
...

    {
        ...
        for (unsigned int x = 0;
             x < (unsigned int)target->e_shentsize * target->e_shnum;
             x += target->e_shentsize) {
            Elf32_Shdr *shdr =
                (Elf32_Shdr *)((uintptr_t)target + (target->e_shoff + x));
    ...
    if (!shstrtab) {
        debug_print(ERROR,
                    "Could not locate module section header string table.");
        goto mod_load_error_unload;
    }

    {
        for (unsigned int x = 0;
            ...
            if (shdr->sh_type == SHT_STRTAB &&
                (!strcmp((char *)((uintptr_t)shstrtab + shdr->sh_name),
                         ".strtab"))) {
...

    {
        ...
        for (unsigned int x = 0;
            ...
            Elf32_Shdr *shdr =
                (Elf32_Shdr *)((uintptr_t)target + (target->e_shoff + x));
            if ((!strcmp((char *)((uintptr_t)shstrtab + shdr->sh_name),
                         "moddeps"))) {
    ...
    {
        Elf32_Sym *table =
            (Elf32_Sym *)((uintptr_t)target + sym_shdr->sh_offset);
        while ((uintptr_t)table - ((uintptr_t)target + sym_shdr->sh_offset) <
               sym_shdr->sh_size) {
            if (table->st_name) {
                if (ELF32_ST_BIND(table->st_info) == STB_GLOBAL) {
                    char *name =
                        (char *)((uintptr_t)symstrtab + table->st_name);
                    if (table->st_shndx == 0) {
                        if (!hashmap_get(symboltable, name)) {
                            debug_print(
                                ERROR, "Unresolved symbol in module: %s", name);
                            undefined = 1;
                        }
                    } else {
                        Elf32_Shdr *s = NULL;
                        {
                            ...
                            int set = 0;
                            for (unsigned int x = 0;
                                 x < (unsigned int)target->e_shentsize *
                                         target->e_shnum;
                                 x += target->e_shentsize) {
                                Elf32_Shdr *shdr =
                                    (Elf32_Shdr *)((uintptr_t)target +
                                                   (target->e_shoff + x));
                                if (i == table->st_shndx) {
                                    set = 1;
                                    s = shdr;
                            ...
                            /*
                             * Common symbols
                             * If we were a proper linker, we'd look at a bunch
                             * of objects to find out if one of them defined
                             * this, but instead we have a strict hierarchy of
                             * symbol resolution, so we know that an undefined
                             * common symbol at this point should be immediately
                            ...
                            if (!set && table->st_shndx == 65522) {
                                if (!hashmap_get(symboltable, name)) {
                                    ...
                                    debug_print(NOTICE, "point %s to 0x%x",
                                                name, (uintptr_t) final);
                                    hashmap_set(symboltable, name,
                                                (void *) final);
                                    hashmap_set(local_symbols, name,
                                                (void *) final);
                                }
                            }
                        }
                    ...
                    Elf32_Shdr *s = NULL;
                    {
                        ...
                        if (!set && table->st_shndx == 65522) {
                            if (!hashmap_get(symboltable, name)) {
                                ...
                                debug_print(NOTICE, "point %s to 0x%x", name,
                                            (uintptr_t) final);
                                hashmap_set(local_symbols, name,
                                            (void *) final);
                            }
    ...
    if (undefined) {
        debug_print(ERROR,
                    "This module is faulty! Verify it specifies all of its");
            ...
            if (shdr->sh_type == SHT_REL) {
                ...
                while ((uintptr_t)table - (shdr->sh_addr) < shdr->sh_size) {
                    ...
                    Elf32_Shdr *rs =
                        (Elf32_Shdr *)((uintptr_t)target +
                                       (target->e_shoff +
                                        shdr->sh_info * target->e_shentsize));
                    ...
                    if (ELF32_ST_TYPE(sym->st_info) == STT_SECTION) {
                        ...
                        char *name =
                            (char *)((uintptr_t)symstrtab + sym->st_name);
                        ptr = (uintptr_t *)(table->r_offset + rs->sh_addr);
                        addend = *ptr;
                        place = (uintptr_t)ptr;
                        if (!hashmap_get(symboltable, name)) {
                            if (!hashmap_get(local_symbols, name)) {
                                debug_print(ERROR, "Wat? Missing symbol %s",
                                            name);
                                debug_print(ERROR, "Here's all the symbols:");
                            } else {
                                symbol =
                                    (uintptr_t)hashmap_get(local_symbols, name);
                            }
                        } else {
...
void modules_install(void) {
    ...
    while ((uintptr_t)k < (uintptr_t)&kernel_symbols_end) {
        ...
        k = (kernel_symbol_t *)((uintptr_t)k + sizeof(kernel_symbol_t) +
                                strlen(k->name) + 1);
    }

    /* Also add the kernel_symbol_start and kernel_symbol_end (these were
     * excluded from the generator) */
...
/* Accessors. */
hashmap_t *modules_get_list(void) { return modules; }

hashmap_t *modules_get_symbols(void) { return symboltable; }
