...
void process_dump(struct process *p) {
    struct x86_stack *s =
        (struct x86_stack *)(INTERRUPT_STACK_TOP - sizeof(*s));
    kprintf("kstack: %x\n", p->kstack);
    kprintf("stackp: %x\n", p->kstack_ptr);
    kprintf("eax: %x     cs: %x\n", s->regs1.eax, s->cs);
    kprintf("ebx: %x     ds: %x\n", s->regs1.ebx, s->ds);
...
int process_kill(uint32_t pid) {
    if (pid > 0 && pid <= PROCESS_MAX_PID && pid != 1) {
        ...
        if (dead) {
            kprintf("successfully killed the specified process\n");
        ...
        kprintf("kill: cannot kill the specified process: you tried to kill "
                "the core process\n");
        return 1;
    } else {
        return 1;
