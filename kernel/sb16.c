...
bool SB16_Reset(void)
{
    ...
    // send reset command
    outb(1, sb16BaseAddress + DSP_RESET);

    // wait (much more than) 3.3 microseconds
    wait_for_io(1);

    outb(0, sb16BaseAddress + DSP_RESET);
...
void SB16_Init(void)
{
    kprintf("[sb16] Checking for a SoundBlaster 16 compatible device...\n");
    ...
    if (!sb16Present)
    {
        kprintf("[sb16] No SoundBlaster 16 compatible device was found.\n");
        return;
    }
    ...
    kprintf("[sb16] SoundBlaster 16 compatible device found at %x, DSP Version %d.%d, initialized.\n", sb16BaseAddress,sb16VersionMajor,sb16VersionMinor);

    interrupt_register(SB16_Interrupt_Handler, sb16IRQ);
    // Enable the SB16's IRQ
    interrupt_enable(sb16IRQ);
}
