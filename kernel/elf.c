...
noexec:
    kprintf("%s isn't an ELF executable or isn't designed for i386 architecture\n", filename);
    fs_dirent_close(d);
...
uint8_t elf32_reloc_apply(void* image, elf_reloc_t* rel, uint32_t symtab_idx, uint32_t t_sect_idx)
{
    ...
    ignore_symbol:
        ...
        default:
            kprintf("Tipo desconhecido de relocalizacao em 'ELF Relocatable'.\n");
            return RELOC_ERROR;
            break;
        }
        return RELOC_OK;
