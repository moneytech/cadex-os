...
int elf_load(struct process *p, const char *filename, addr_t *entry) {
    ...
    if (actual != sizeof(program))
    	...
    	// dbg_printf("elf: text %x bytes from offset %x at address %x length %x\n",program.file_size,program.offset,program.vaddr,program.memory_size);

    if (program.type != ELF_PROGRAM_TYPE_LOADABLE ||
        program.vaddr < PROCESS_ENTRY_POINT ||
        program.memory_size > 0x8000000 ||
    ...
    for (i = 0; i < header.shnum; i++) {
        ...
        if (section.type == ELF_SECTION_TYPE_BSS) {
            uint32_t limit =
                section.address + section.size - PROCESS_ENTRY_POINT;
...
noload:
    kprintf("%s failed to load correctly\n", filename);
    fs_dirent_close(d);
    return KERROR_NOT_FOUND;

noexec:
    kprintf(
        "%s isn't a valid ELF executable or is designed for an architecture other than i386\n",
        filename);
    fs_dirent_close(d);
    return KERROR_NOT_EXECUTABLE;

mustdie:
    kprintf("%s did not load correctly\n", filename);
    fs_dirent_close(d);
    return KERROR_EXECUTION_FAILED;
}
uint8_t elf32_get_section_by_name(void *image, char *sect_name,
                                  elf_sect_t **sect) {
    ...
    str_sect = (elf_sect_t *)(image + elf->shtab_offset +
                              (elf->sh_size * elf->shstrtab_index));
    ...
    for (i = 0; i < elf->num_sects; i++) {
        ...
        index = ((elf_sect_t *)(image + elf->shtab_offset + (elf->sh_size * i)))
                    ->sect_name;
        if (!strcmp((char *)(strings + index), sect_name)) {
            *sect =
                (elf_sect_t *)(image + elf->shtab_offset + (elf->sh_size * i));
            return 1;
        }
    }
...
/*
    Atribui � 'sect' a i-�sima ocorrencia de se��o do 'type' e 'flags'
   requerido. Se 'flags' < 0 ele ser� ignorado.
*/
uint8_t elf32_get_sections_i(void *image, uint32_t index, uint32_t type,
                             int32_t flags, elf_sect_t **sect) {
    elf_file_t *elf = (elf_file_t *)image;
    elf_sect_t *tmp;
    uint32_t i;
...
uint8_t elf32_get_symbol_i(void *image, uint32_t sym_idx, elf_sect_t *symtab,
                           elf_sym_t **symbol) {
    ...
    *symbol =
        (elf_sym_t *)(image + symtab->offset + symtab->ent_size * sym_idx);
    return 1;
}

uint8_t elf32_get_symbol_by_name(void *image, const char *sym_name,
                                 int32_t type, elf_sect_t *symtab,
                                 elf_sect_t *strtab, elf_sym_t **symbol) {
    elf_sym_t *sym;
    uint32_t i;
    int8_t *strings;
...
*/
uint8_t elf32_search_symbol(void *image, const char *name, uint32_t type,
                            elf_sym_t **symbol) {
    elf_sect_t *sect, *strtab;
    uint32_t i;

    for (i = 0; i < elf32_get_sections(image, SHT_SYMTAB, -1);
         i++) // pesquisa em todas tabelas de s�mbolos.
    {
        if (elf32_get_sections_i(image, i, SHT_SYMTAB, -1,
                                 &sect)) // carrega symtab #i
            if (elf32_get_section_i(image, sect->link,
                                    &strtab)) // carrega strtab correspondente.
                if (elf32_get_symbol_by_name(image, name, type, sect, strtab,
                                             symbol)) // procura pelo s�mbolo.
                    return 1;
    }
    return 0;
}

uint8_t elf32_get_name(void *image, uint32_t index, elf_sect_t *strtab,
                       const char **name) {
    int8_t *strings;
...
uint8_t elf32_reloc_apply(void *image, elf_reloc_t *rel, uint32_t symtab_idx,
                          uint32_t t_sect_idx) {
    ...
    if (!elf32_get_section_i(
            image, symtab_idx,
            &symtab)) // carrega 'Symbol Table' associada � 'Relocation Table'.
        return RELOC_ERROR_SYMTAB;

    if (!elf32_get_section_i(image, t_sect_idx,
                             &sect)) // carrega se��o alvo da relocaliza��o.
        return RELOC_ERROR_TARGET;

    // kprintf("symtab.type=%d tsect.type=%d symbol.sect=%d symidx=%d\n",
    // symtab->type & 0xff, sect->type & 0xff, symbol->section & 0xffff,
    // symtab_idx);

    if (rel->symtab_index ==
        0) // Refer�ncia: (If the index is STN_UNDEF, the undefined symbol
           // index, the relocation uses 0 as the "symbol value.")
    {
        sym_val = 0;
        goto ignore_symbol;
    } else {
        if (!elf32_get_symbol_i(
                image, rel->symtab_index, symtab,
                &symbol)) // carrega o s�mbolo a ser relocalizado.
    ...
    ignore_symbol:
        rel_val = (uint32_t *)(image + sect->offset +
                               rel->adr); // Endere�o da relocaliza��o
        if (symbol->section == SHN_SOMBRA)
            *rel_val = (uint32_t)sym_val + *rel_val;
        else
            *rel_val = (uint32_t)((uint32_t)(image + sym_val + sect2->offset) +
                                  *rel_val);
        ...
        switch (rel->type) {
            ...
            default:
                kprintf("Tipo desconhecido de relocalizacao em 'ELF "
                        "Relocatable'.\n");
                return RELOC_ERROR;
                break;
        }
...
uint8_t elf32_check(void *image, uint8_t type) {
    ...
    if (elf->bitness != 1 || elf->endian != 1 ||
        elf->elf_ver_1 != ELF_HEADER_VERSION || elf->file_type != type ||
        elf->machine != ELF_HEADER_MACHINE_I386 || elf->elf_ver_2 != 1)
        return 0;
