...
void _page_map(uint32_t p_addr, uint32_t v_addr, uint32_t length) {
    ...
    while (pages > 0) {
        ...
        table[p_idx] = (p_addr & 0xFFFFF000) | PAGE_FLAG_PRESENT |
                       PAGE_FLAG_RW | PAGE_FLAG_BITMAP_END |
                       PAGE_FLAG_BITMAP_START;
...
uint32_t acpi_init() {
    ...
    for (ptr = 0; ptr < 0x100000; ptr += 16) {
        ...
        if (!strncmp((char *)ptr, "RSD PTR ", 8)) {
            ...
            // return_val_if_fail(acpi_checksum(rsdp,sizeof(struct
            // RSDPDescriptor),rsdp->Checksum), 0);
            ...
            kprintf("found (OEM: %s Rev. %d) ", rsdp->OEMID, rsdp->Revision);

            rsdt = (struct RSDT *)rsdp->RsdtAddress;
            kprintf("RsdtAddr: 0x%x Signature: ", rsdp->RsdtAddress);
            _page_map(rsdp->RsdtAddress, rsdp->RsdtAddress,
                      0x1000); // make rsdt header accessible
            ...
            for (i = 0; i < i_max; i++) {
                ...
                _page_map(rsdt->PointerToOtherSDT[i],
                          rsdt->PointerToOtherSDT[i], 0x1000);
                _page_map(rsdt->PointerToOtherSDT[i],
                          rsdt->PointerToOtherSDT[i], sdt->Length);
                ...
                switch (*((uint32_t *)sdt->Signature)) {
                    case 'TDSS': // same than DSDT
                        ...
                        if (S5Block != NULL) {
                            ...
                            S5Block +=
                                ((*S5Block & 0xC0) >> 6) + 2; // skip PkgLength
                            ...
                            // kprintf("SLP_TYPa=0x%x SLP_TYPb=0x%x ", SLP_TYPa,
                            // SLP_TYPb);
                        }
                        break;
                    case 'PCAF': // "FACP"
                        ...
                        interrupt_register(
                            facp->SCI_Interrupt,
                            acpi_handler); // install ACPI IRQ handler

                        kprintf("Profile=%d ",
                                facp->PreferredPowerManagementProfile);
                        // kprintf("ResetReg=0x%x(0x%x)\n",
                        // facp->ResetReg.Address, facp->ResetValue);

                        dsdt = (struct ACPISDTHeader *)facp->Dsdt;
                        if (dsdt != NULL) {
                            _page_map(facp->Dsdt, facp->Dsdt,
                                      0x1000); // make DSDT header accessible
                            _page_map(facp->Dsdt, facp->Dsdt,
                                      dsdt->Length); // map the correct size
                        }
                        ...
                        if ((*((uint32_t *)dsdt) == 'TDSD')) { // "DSDT"
                            ...
                            if (S5Block != NULL) {
                                // kprintf("\\_S5 block found!");
                                S5Block += 4; // skip \_S5_ signature
                                S5Block += ((*S5Block & 0xC0) >> 6) +
                                           2; // skip PkgLength

                                if (*S5Block ==
                                    0x0A) // skip BytePrefix AML OpCode
                                    S5Block++;
                                SLP_TYPa = *S5Block; // & 0x7; // 3-bit
                                S5Block++;
                                ...
                                // kprintf("SLP_TYPa=0x%x SLP_TYPb=0x%x ",
                                // SLP_TYPa, SLP_TYPb);
                            }
                        } else {
                    ...
                    case 'TEPH': // HPET
                        ...
                        break;
                    default:
                        // kprintf("[-%c%c%c%c] ", sdt->Signature[0],
                        // sdt->Signature[1], sdt->Signature[2],
                        // sdt->Signature[3]);
                        break;
                }
            }
            ...
            kprintf(
                "[acpi] SCI_Interrupt=%d SMI_CommandPort=0x%x ShutDown=%c\n",
                facp->SCI_Interrupt, facp->SMI_CommandPort,
                (S5Block == NULL) ? 'N' : 'Y');
            return ptr;
        }
    }
    kprintf("no ACPI devices available\n");
    return 0;
}
