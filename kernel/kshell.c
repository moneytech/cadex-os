		...
		if(fs) {
			...
			if(v) {
				...
				if(d) {
					...
					printf("mount.exe: couldn't find root dir on %s unit %d!\n",device_name(dev),device_unit(dev));
					return -1;
				}
				fs_volume_close(v);
			} else {
				printf("mount.exe: couldn't mount %s on %s unit %d\n",fs_type,device_name(dev),device_unit(dev));
				return -1;
			}
		} else {
			printf("mount.exe: invalid fs type: %s\n", fs_type);
			return -1;
		}
		device_close(dev);
	} else {
		printf("mount.exe: couldn't open device %s unit %d\n",devname,unit);
		return -1;
	}
	...
	if(d) {
		...
		if(buffer) {
			...
			if(length>=0) {
				...
				printf("list.exe: %s is not a directory\n", path);
			}
			kfree(buffer);
		}
	} else {
...
static int kshell_execute(int argc, const char **argv)
{
	...
	if(!strcmp(cmd, "start")) {
		...
		if(argc > 1) {
			...
			process_yield();
			printf("exec.exe: error: couldn't execute %s\n", argv[1]);
		} else {
			printf("exec.exe: requires argument.\n");
		}
		...
		if(argc==4) {
			...
			if(str2int(argv[2], &unit)) {
				...
				printf("mount.exe: expected unit number but got %s\n", argv[2]);
			}
		} else {
			printf("mount.exe: requires device, unit, and fs type\n");
		}
		...
		if(argc > 1) {
			...
			if(str2int(argv[1], &pid)) {
				...
				printf("reap.exe: expected process id but got %s\n", argv[1]);
			}
		} else {
			printf("reap.exe: requires argument\n");
		}
		...
		if(argc > 1) {
			...
			if(str2int(argv[1], &pid)) {
				...
				printf("kill.exe: expected process id number but got %s\n", argv[1]);
			}
		} else {
			printf("kill.exe: you need to specify the PID of which program to kill.\n");
		}
		...
		if(argc == 3) {
			...
			if(dir) {
				...
				if(!n) {
					printf("mkdir.exe: couldn't create %s\n",argv[2]);
				}
				fs_dirent_close(n);
				fs_dirent_close(dir);
			} else {
				printf("mkdir.exe: couldn't open %s\n",argv[1]);
			}
		} else {
			...
			if(str2int(argv[2], &unit)) {
				...
				if(f) {
					...
					if(d) {
						...
						printf("format.exe: couldn't open device %s unit %d\n",argv[1],unit);
					}
				} else {
					printf("format.exe: invalid fs type: %s\n", argv[3]);
				}
			} else {
				printf("format.exe: expected unit number but got %s\n", argv[2]);
			}
		}
		...
		if(argc==3) {
			...
			printf("install.exe: expected unit #s for cdrom and disk\n");
		}
		...
		if(argc == 3) {
			...
			if(dir) {
				...
				if(result<0) {
					printf("remove.exe: couldn't remove %s\n",argv[2]);
				}
				fs_dirent_close(dir);
			} else {
				printf("remove.exe: couldn't open %s\n",argv[1]);
			}
		} else {
