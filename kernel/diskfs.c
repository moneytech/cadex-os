...
static uint32_t diskfs_data_block_alloc(struct fs_volume* v)
{
    ...
    kprintf("diskfs: error: cannot perform action: disk full.\n");
    kprintf("[diskfs] cannot perform action: disk full\n");
    page_free(b);
    return 0;
}
...
static int diskfs_inumber_alloc(struct fs_volume* v)
{
    ...
    kprintf("diskfs: warning: out of inodes!\n");
    kprintf("[diskfs] out of disk inodes\n");
    page_free(b);
    return 0;
}
...
int diskfs_volume_format(struct device* device)
{
    ...
    kprintf("diskfs: writing superblock\n");
    ...
    kprintf("diskfs: flushing buffer cache...");
