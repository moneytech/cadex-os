...
static int ata_probe_internal( int id, int kind, int *nblocks, int *blocksize, char *name )
{
	...
	if(kind==ATA_COMMAND_IDENTIFY || kind==0) {
		...
		if(result) {
			*nblocks = buffer[1] * buffer[3] * buffer[6];
			printf("[HARDWARE] hdd: %d logical cylinders\n", buffer[1]);
			printf("[HARDWARE] hdd: %d logical heads\n", buffer[3]);
			printf("[HARDWARE] hdd: %d logical sectors/track\n", buffer[6]);
	...
	printf("[SYS] %s unit %d: %s %u sectors %u MB %s\n",
