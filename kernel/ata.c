...
static int ata_probe_internal(int id, int kind, int *nblocks, int *blocksize,
    ...
    if (t == 0xff) {
        ...
        kprintf("[ata] nothing attached on unit %d\n", id);
        return 0;
    }
    ...
    if (kind == ATA_COMMAND_IDENTIFY || kind == 0) {
        ...
        if (result) {
            *nblocks = buffer[1] * buffer[3] * buffer[6];
            kprintf("[ata] hdd: %d logical cylinders\n", buffer[1]);
            kprintf("[ata] hdd: %d logical heads\n", buffer[3]);
            kprintf("[ata] hdd: %d logical sectors/track\n", buffer[6]);
    ...
    if (kind == ATAPI_COMMAND_IDENTIFY || (kind == 0 && result == 0)) {
        ...
        if (result) {
            // TODO: use SCSI sense to get media size
            *nblocks = 337920;
...
int ata_probe(int id, int *nblocks, int *blocksize, char *name) {
    return ata_probe_internal(id, ATA_COMMAND_IDENTIFY, nblocks, blocksize,
                              name);
}
