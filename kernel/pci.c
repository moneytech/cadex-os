...
void PCI_CheckDevice(uint8_t bus, uint8_t device) {
    ...
    kprintf("[pci] Found PCI Device with vendor ID %x, (%s)\n", vendorID,
            PCI_GetVendorName(vendorID));
...
void PCI_DelegateToDriver(uint8_t bus, uint8_t slot, uint8_t function,
    ...
    if (baseClass == PCI_BASE_CLASS_MASS_STORAGE &&
        subClass == PCI_SUBCLASS_IDE_CONTROLLER) {
...
uint8_t PCI_GetCapabilitiesPointer(uint8_t bus, uint8_t slot,
                                   uint8_t function) {
    uint16_t pointer =
        PCI_ConfigReadWord(bus, slot, function, CAP_POINTER_OFFSET) & ~3;
    return pointer & 0xFF;
}
...
void PCI_ConfigWriteDWord(uint8_t bus, uint8_t slot, uint8_t func,
...
uint8_t PCI_GetInterruptLine(uint8_t bus, uint8_t slot, uint8_t function) {
    return PCI_ConfigReadWord(bus, slot, function, INTERRUPT_LINE_OFFSET) &
           0xFF;
}
...
uint8_t PCI_GetProgrammingInterface(uint8_t bus, uint8_t slot,
    ...
    return (uint8_t)(
        PCI_ConfigReadWord(bus, slot, function, VENDOR_ID_OFFFSET) >> 8);
}
...
void PCI_SetCommand(uint8_t bus, uint8_t device, uint8_t function,
    ...
    uint32_t dataDWORD =
        PCI_ConfigReadDWord(bus, device, function, COMMAND_OFFSET);
