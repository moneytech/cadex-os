...
void is_valid_test()
{
    kprintf("Testing PATH validations...\n");
    char* s = "/root/is-valid.path";
    if (!is_valid_path(s))
        kprintf("ERROR: Did not validate %s\n", s);
    ...
    s = "/root/isn't/valid:";
    if (is_valid_path(s))
        kprintf("ERROR: Did not invalidate %s\n", s);

    kprintf("Testing Tag Validations...\n");
    s = "TAG";
    if (!is_valid_tag(s))
        kprintf("ERROR: Did not validate %s\n", s);
    s = "abcdefghijklmnopqrstuvwxyz";
    ...
    kprintf("Testing Location Validations...\n");
