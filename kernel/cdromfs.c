...
static struct fs_dirent *cdrom_dirent_lookup(struct fs_dirent *dir,
    ...
    int nsectors = dir->size / CDROMFS_BLOCK_SIZE +
                   (dir->size % CDROMFS_BLOCK_SIZE ? 1 : 0);
        ...
        while (d->descriptor_length > 0) {
            ...
            if (!strncmp(name, dname, dname_length)) {
                ...
                r = cdrom_dirent_create(
                    dir->volume, d->first_sector_little, d->length_little,
                    d->flags & ISO_9660_EXTENT_FLAG_DIRECTORY);
                page_free(temp);
                return r;
            }
            d = (struct iso_9660_directory_entry *)((char *)d +
                                                    d->descriptor_length);
        }
    }
...
static struct fs_volume *cdrom_volume_open(struct device *device) {
    ...
    // kprintf("cdfs: scanning %s unit
    // %d...\n",device_name(device),device_unit(device));
    kprintf("[cdfs] scanning %s unit %d...\n", device_name(device),
            device_unit(device));
    int j;

    for (j = 0; j < 16; j++) {
        kprintf("[cdfs] checking volume %d...\n", j);
        ...
        if (d->type == ISO_9660_VOLUME_TYPE_PRIMARY) {
            ...
            kprintf("[cdfs] mounted filesystem on %s unit%d\n",
                    device_name(v->device), device_unit(v->device));
    ...
    kprintf("cdfs: no filesystem found\n");
    dbg_printf("[cdfs] no filesystem found");
    return 0;
}

static struct fs_dirent *cdrom_volume_root(struct fs_volume *v) {
    return cdrom_dirent_create(v, v->cdrom.root_sector, v->cdrom.root_length,
                               1);
}
...
static struct fs cdrom_fs = {"cdfs", &cdrom_ops, 0};
