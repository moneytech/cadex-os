...
void page_init() {
    ...
    kprintf("[page] memory: %d MB (%d KB) total\n",
    ...
    kprintf("[page] memory: %d bits %d bytes %d cells %d pages\n", freemap_bits,
    ...
    kprintf("[page] memory: %d MB (%d KB) available\n",
            (pages_free * PAGE_SIZE) / MEGA, (pages_free * PAGE_SIZE) / KILO);
}
...
void *page_alloc(bool zeroit) {
    ...
    if (!freemap) {
        kprintf("memory: not initialized yet!\n");
        return 0;
    }
    ...
    kprintf("memory: WARNING: everything allocated\n");
    halt();
