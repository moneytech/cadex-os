 ...
 *
 * This file includes all necessary headers for making a standard TrueGL GLX
 * application
 */
...
auto glx_get_config = [](GraphicsContext context) {
    if (current_glx_config->window_props->root_window) {
        // If the current glx window config's root_window property is 1 then
        // return the current glx config
